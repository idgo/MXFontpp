...
class FactTrainer(BaseTrainer):
    ...
    def train(self, loader, st_step=0, max_step=100000):
        ...
        losses = utils.AverageMeters("g_total", "pixel", "disc", "gen", "fm", "indp_exp", "indp_fact",
                                     ...
                                     "ac_gen_s", "ac_gen_c", "cross_ac_gen_s", "cross_ac_gen_c", "contrast_loss")
        # discriminator stats
        ...
        #for batch in cyclize(loader):
            ...
            last_feat_shape = style_facts_s['last'].shape[2:]
            skip_feat_shape = style_facts_s['skip'].shape[2:]
            self.add_contrast_loss(
                [style_facts_s["last"].view(B*n_s*self.gen.n_experts, -1).mean(dim=0), char_facts_c["last"].view(B*n_c*self.gen.n_experts, -1).mean(dim=0)],
                [char_facts_s["last"].view(B*n_c*self.gen.n_experts, -1).mean(dim=0), style_facts_c["last"].view(B*n_s*self.gen.n_experts, -1).mean(dim=0)],
            )
            # self.add_contrast_loss(
            #     [style_facts_s["last"].view(B*n_s*self.gen.n_experts, *last_feat_shape), style_facts_c["last"].view(B*n_s*self.gen.n_experts, *last_feat_shape)],
            #     [style_facts_s["skip"].view(B*n_s*self.gen.n_experts, *skip_feat_shape), style_facts_c["skip"].view(B*n_s*self.gen.n_experts, *skip_feat_shape)],
            #     [char_facts_s["last"].view(B*n_c*self.gen.n_experts, *last_feat_shape), char_facts_c["last"].view(B*n_c*self.gen.n_experts, *last_feat_shape)],
            #     [char_facts_s["skip"].view(B*n_c*self.gen.n_experts, *skip_feat_shape), char_facts_c["skip"].view(B*n_c*self.gen.n_experts, *skip_feat_shape)],
                
            #     [style_facts_s["last"].view(B*n_s*self.gen.n_experts, *last_feat_shape), char_facts_c["last"].view(B*n_c*self.gen.n_experts, *last_feat_shape)],
            #     [style_facts_s["skip"].view(B*n_s*self.gen.n_experts, *skip_feat_shape), char_facts_c["skip"].view(B*n_c*self.gen.n_experts, *skip_feat_shape)],
            #     [char_facts_s["last"].view(B*n_c*self.gen.n_experts, *last_feat_shape), style_facts_c["last"].view(B*n_s*self.gen.n_experts, *last_feat_shape)],
            #     [char_facts_s["skip"].view(B*n_c*self.gen.n_experts, *skip_feat_shape), style_facts_c["skip"].view(B*n_s*self.gen.n_experts, *skip_feat_shape)],
            # )
    ...
    def add_contrast_loss(self, *exp_pairs):
        """计算对比loss
        """
        crit = self.min_cossim
        for pair in exp_pairs:
            contrast_loss = self.add_loss(pair, self.g_losses, "contrast_loss", self.cfg["contrast_loss_w"], crit)
            if self.step % 1000 == 0:
                print(contrast_loss, pair[0].isnan().sum())
